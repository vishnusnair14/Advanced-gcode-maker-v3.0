...
class Append_Compiler:
    """
    The Append Compiler class handles the process of drawing geometric objects using interface commands and assembling
    ...
    def compile_to_file(self, file_name: str, passes=1):
        ...
        with open(file_name, 'a') as file:
            file.write("(Generated by Gcode_Maker_3.0)\n(by vishnus_technologies)\n"+self.compile(passes=passes)+"\n\n")
        
        # Corrects the '\n' string if found instead of '\n' literal for nextline
        cv3 = open(file_name, "r") 
        data1 = cv3.read()
        data1 = data1.replace('\\n','\n')
        cv3.close()
        cv4 = open(file_name, "w+")
        cv4.write(data1)
        cv4.close()
        
        # Clears all blank line(s) in gcode file
        with open(file_name, 'r+') as fd1:
            lines = fd1.readlines()
            fd1.seek(0)
            fd1.writelines(line for line in lines if line.strip())
            fd1.truncate()
